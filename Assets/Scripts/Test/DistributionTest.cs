...
namespace Test {
    public class DistributionTest {
        ...
        /* Probabilities */
        private readonly int _outermostTileThreeNeighbourProbability = 10;
        private readonly int _threeNeighbourPropagationProbability = 50;

        public Dictionary<(int, int), int> TestGenerateDistributedNeighbourNums() {
            ...
            // Pattern 1
            TestDetermineOutermostRingTiles();
            
            // Pattern 2 & 3
            TestDetermineInnerTiles();
            
            // Pattern 4
            TestPlaceAdjacentFourNeighbourTilesInCentre();

            // Return the final map tile neighbour information
            return _distributedNeighbourNums;
        }

        private void TestDetermineOutermostRingTiles() {
            ...
            foreach (var tile in outerTiles) {
                ...
                if (!TestHasAdjacentThreeNeighbourTiles(tile) &&
                    _random.Next(100) < _outermostTileThreeNeighbourProbability) {
        ...
        private double TestWeightedThreeNeighbourProb((int, int) tile) {
        ...
        private void TestDetermineInnerTiles() {
            for (int x = 1; x <= 9; x++) {
                for (int y = 1; y <= 9; y++) {
                    ...
                    if (_random.Next(100) < TestWeightedThreeNeighbourProb((x, y))) {
                        _distributedNeighbourNums[(x, y)] = 3;
                        
                        // Local propagation mechanism
                        if (_random.Next(100) < _threeNeighbourPropagationProbability) {
        ...
        private void TestPlaceAdjacentFourNeighbourTilesInCentre() {
         ...
         */
        private bool TestHasAdjacentThreeNeighbourTiles((int, int) tile) {
            if (tile.Item1 == 0) {
                if (tile.Item2 == 0) {
